...
package object barneshut {
  ...
  case class Fork(nw: Quad, ne: Quad, sw: Quad, se: Quad) extends Quad {
    val quad = List(nw, ne, sw, se)

    val centerX: Float = (nw.centerX + ne.centerX) / 2
    val centerY: Float = (nw.centerY + sw.centerY) / 2
    val size:    Float = nw.size * 2
    val mass:    Float = quad.foldLeft(0f)(_+_.mass)

    val massX: Float =
      if (mass == 0) centerX
      else quad.foldRight(0f)((qd, mass) => qd.mass * qd.massX + mass) / mass

    val massY: Float =
      if (mass == 0) centerY
      else quad.foldRight(0f)((qd, mass) => qd.mass * qd.massY + mass) / mass

    val total:   Int   = quad.foldLeft(0)(_ + _.total)
  ...
  extends Quad {
    val mass: Float = bodies.foldRight(0f)((body, mass)  => body.mass + mass)
    val (massX: Float, massY: Float) = (
      bodies.foldRight(0f)((body, massX) => body.mass * body.x + massX) / mass,
      bodies.foldRight(0f)((body, massY) => body.mass * body.y + massY) / mass
    )
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) =>
          ...
          val distance_ = distance(quad.massX, quad.massY, x, y)

          if (quad.size / distance_ < theta) addForce(quad.mass, quad.massX, quad.massY)
